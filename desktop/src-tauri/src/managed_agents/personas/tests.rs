...
#[test]
fn merge_personas_seeds_no_built_ins_on_empty_store() {
    let (records, changed) = merge_personas(Vec::new(), "2026-03-19T00:00:00Z");

    assert!(!changed);
    assert!(records.is_empty());
    assert!(BUILT_IN_PERSONAS.is_empty());
}
...
#[test]
fn merge_personas_demotes_and_retires_former_starter_edits() {
    ...
    let fizz = records
        ...
        .expect("fizz record should be retained");
...
#[test]
fn merge_personas_does_not_repromote_retired_starters() {
    let mut edited_builtin = custom_persona("builtin:fizz", "My Fizz");
    ...
    assert!(changed); // soft-retire inactive + suffix
    let fizz = records
        .iter()
        .find(|record| record.id == "builtin:fizz")
        .expect("fizz record should be retained");
...
#[test]
fn merge_personas_retires_legacy_builtins_without_seeding_starters() {
    ...
    assert!(changed);
    assert!(
        records.iter().all(|r| r.id != "builtin:fizz"),
        "retired starters must not be re-seeded"
    );
...
// ── Built-in seed surface ─────────────────────────────────────────────────────

#[test]
fn built_in_persona_records_is_empty() {
...
#[test]
fn unknown_persona_resolves_to_default_agent_command() {
    // With no built-in seed table, unknown persona ids still fall through to
    // the bundled default harness.
    let records = built_in_persona_records("2026-01-01T00:00:00Z");
    assert_eq!(
        effective_agent_command(Some("custom:any"), &records, None),
