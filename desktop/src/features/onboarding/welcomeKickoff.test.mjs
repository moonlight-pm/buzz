...
test("resolveWelcomeAgentSet returns null when no welcome team starters are configured", () => {
  // Moonlight: WELCOME_TEAM_STARTERS is empty (starters retired). Kickoff cannot
  // resolve a lead/teammate set from personas even if former builtin agents exist.
  assert.equal(resolveWelcomeAgentSet([bumble, fizz, honey]), null);
  assert.equal(resolveWelcomeAgentSet([fizz, honey]), null);
  assert.equal(resolveWelcomeAgentSet([]), null);
});
